...
class Validate
{
    ...
    static public function isPhone($phone)
    {
        ...
        $regex = '/^' . Configuration::get('_REGEX_PHP_PHONE', '[0-9]{10}') . '$/';
...
//                $regex = Configuration::get('_REGEX_PHP_PHONE');


    }

    static public function validateInternationalPhone( $str) {
        $str = trim( $str );
    ...
    /**
     * Check phone number validity, or try to reformat it
     *
     * @param string $phone
     * @return bool|string
     */
    public static function validatePhone($number, $prefix = '33')
    {
        $phone = preg_replace('/[^0-9\+]/', '', $number);
        if(preg_match('/^(\+'.$prefix.'|0)[0-9]{9}$/', $phone, $matches)) {
            return $phone;
        }
        return false;
    }
    ...
    {
        return preg_match('/^[a-z\-]{2,5}$/ui', $isoCode);
    }
    ...
    {
        return preg_match('/^[^!<>,;?=+()@"Â°{}_$%:]{3,32}$/ui', $discountName);
    }
    ...
    {
        return preg_match('/^[^!<>;?=+@#"Â°{}_$%0-9]*$/ui', $city);
    }
    ...
    {
        return preg_match('/^\_{1}[a-zA-Z_]{1,50}\_{1}$/', $configName);
    }
