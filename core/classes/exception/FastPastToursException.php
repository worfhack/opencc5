 ...
 * Time: 13:20
 */
class FastPastToursException extends Exception
{
    /**
    ...
    public function displayMessage()
    {
        ...
        if (_PS_MODE_DEV_ || defined('_PS_ADMIN_DIR_')) {
            ...
            foreach ($this->getTrace() as $id => $trace) {
                $relative_file = (isset($trace['file'])) ? ltrim(str_replace(array(ROOT_DIR, '\\'), array('', '/'), $trace['file']), '/') : '';
            ...
            if (file_exists(ROOT_DIR.'/error500.html')) {
                echo file_get_contents(ROOT_DIR.'/error500.html');
            }
        }
    ...
    protected function logError()
    {
        ...
        $logger->setFilename(ROOT_DIR.'/app/logs/'.date('Ymd').'_exception.log');
    ...
    protected function getExtendedMessage($html = true)
    {
        ...
        return sprintf(
            ...
            $this->getMessage(),
            $this->getLine(),
            ltrim(str_replace(array(ROOT_DIR, '\\'), array('', '/'), $this->getFile()), '/')
        );
    }
}
